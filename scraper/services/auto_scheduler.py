...
class AutoScheduler:
    ...
    def _schedule_if_due(self):
        ...
        if current_time - self.last_runs['missing_fields'] >= 3600:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_missing_fields', limit=50000)
        ...
        # Missing translations every 60 minutes (3600 seconds)
        if current_time - self.last_runs['missing_translations'] >= 3600:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_missing_translations', limit=50000)
        ...
        # Missing comments every 2 hours (7200 seconds)
        if current_time - self.last_runs['missing_comments'] >= 7200:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_missing_comments', limit=50000)
        ...
        # Untranslated comments every 2 hours (7200 seconds)
        if current_time - self.last_runs['untranslated_comments'] >= 7200:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_untranslated_comments', limit=10000)
        ...
        # Untranslated nicknames every 2 hours (7200 seconds)
        if current_time - self.last_runs['untranslated_nicknames'] >= 7200:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_untranslated_nicknames', limit=10000)
        ...
        # Refresh book stats every 15 minutes (900 seconds)
        if current_time - self.last_runs['refresh_book_stats'] >= 900:
            try:
                job_id = self.queue_manager.add_general_job('refresh_book_stats', limit=10000)
        ...
        # Upload images to R2 every 15 minutes (900 seconds)
        if current_time - self.last_runs['upload_images'] >= 900:
            try:
                job_id = self.queue_manager.add_general_job('upload_images', limit=50000)
        ...
        if current_time - self.last_runs['booklist_missing_translations'] >= 7200:
            try:
                job_id = self.queue_manager.add_maintenance_job('check_booklist_missing_translations', limit=5000)
        ...
        # Refresh QQ charts every 24 hours (86400 seconds)
