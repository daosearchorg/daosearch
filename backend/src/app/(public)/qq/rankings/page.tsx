...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const title = `${genderLabel} ${typeLabel} Rankings — QQ Charts`;
  const description = `Browse the top ${typeLabel.toLowerCase()} ${genderLabel.toLowerCase()} web novels from book.qq.com — updated daily. Trending, rising, and all-time favorites on DaoSearch.`;

  return {
    title,
    description,
    keywords: [`${genderLabel.toLowerCase()} web novels`, "qq web novel rankings", "chinese novel rankings", "web novel charts", "daosearch"],
    alternates: { canonical: "/qq/rankings" },
    openGraph: {
      title,
      description,
      type: "website",
