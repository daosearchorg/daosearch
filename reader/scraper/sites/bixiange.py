"""bixiange.me scraper — adapted for httpx + Google Translate proxy."""
...
SPAM_PATTERNS = ["bixiange", "笔下阁"]


class BixiangeScraper(BaseScraper):

    def is_novel_url(self, url: str) -> bool:
        return bool(re.search(r"bixiange\.me/\w+/\d+(?:\.html)?/?$", url))
    ...
    async def scrape_novel_data(self, url: str) -> NovelData:
        ...
        raw_title = self._text(tree, ".desc h1") or self._text(tree, ".detail h1") or self._text(tree, "h1")
        title = re.sub(r"\s*\(\d+-\d+\)\s*$", "", raw_title).strip()

        author = ""
        for span in tree.cssselect(".descTip span"):
        ...
        image_url = ""
        for el in tree.cssselect(".detail img, .cover img, .desc img"):
            src = el.get("src", "")
            if src:
    ...
    async def scrape_chapter(self, url: str) -> ChapterContent:
        ...
        content_el = tree.cssselect("div#mycontent") or tree.cssselect("div.content")
        if not content_el:
            return ChapterContent(title=title, content="", chapter_url=url)

        container = content_el[0]
        for bad in container.cssselect("script, style, div.ad, ins"):
        ...
        lines = [
            ln for ln in lines
            if ln
            and not any(s in ln for s in SPAM_PATTERNS)
            and not re.match(r"^[Pp][Ss][：:]", ln)
